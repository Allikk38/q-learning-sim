...
class World:
    ...
    def _get_occupied_creature_cells(self) -> set:
        """Возвращает множество клеток, занятых агентами и хищниками."""
        occupied = set()
        for agent in self.agents:
            if agent.alive:
                occupied.add((agent.x, agent.y))
        for predator in self.predators:
            occupied.add((predator.x, predator.y))
        return occupied

    def _get_random_empty_cell(self, occupied: set) -> tuple:
        all_cells = [(x, y) for x in range(self.width) for y in range(self.height)]
        creature_cells = self._get_occupied_creature_cells()
        all_occupied = occupied | creature_cells
        free_cells = [c for c in all_cells if c not in all_occupied]
        if not free_cells:
    ...
    def _spawn_initial(self):
        ...
        predator = Predator(agent_id=0, x=cx, y=cy)
        ...
        # 3 агента в разных углах
        spawn_corners = [
            (2, 2),
            (self.width - 3, 2),
            (self.width - 3, self.height - 3)
        ]
        for i, (sx, sy) in enumerate(spawn_corners):
            agent = Creature(agent_id=i, x=sx, y=sy)
            self.agents.append(agent)
            occupied.add((sx, sy))

    def spawn_food(self):
        """Спавнит еду на клетках с высокой плодородностью."""
        for y in range(self.height):
            for x in range(self.width):
                ...
                if cell.type != "empty":
                    continue
                fertility = cell.properties.get("fertility", 0.0)
                if fertility > 0 and random.random() < fertility * 0.3:
                    cell.type = "food"
    ...
    def apply_brush(self, x: int, y: int, fertility: float, radius: int):
        for dy in range(-radius, radius + 1):
            for dx in range(-radius, radius + 1):
                ...
                if dist > radius:
                    continue
                value = fertility * (1 - dist / max(radius, 1))
                self.grid[ny][nx].properties["fertility"] = value
    ...
    def to_delta(self, previous_state: dict) -> list:
        ...
        for agent in current_state["agents"]:
            ...
            if prev_agent:
                if prev_agent["alive"] and not agent["alive"]:
                    deltas.append({"type": "agent_died", "id": agent_id})
        ...
        prev_cells = {(c["x"], c["y"]): c["type"] for c in previous_state.get("cells", [])}
        curr_cells = {(c["x"], c["y"]): c["type"] for c in current_state["cells"]}
        for (x, y), prev_type in prev_cells.items():
            curr_type = curr_cells.get((x, y))
            if prev_type == "food" and curr_type != "food":
                deltas.append({"type": "food_consumed", "x": x, "y": y})
        return deltas
